...
export const CampaignService = defineModule<{ server: MerklServer; request: Request; api: Api }>().create(
  ({ inject }) => {
     ...
     *
     */
    const getQueryFromRequest = inject(["server", "request"]).inFunction(
      ({ request, server }, override?: Parameters<Api["v4"]["campaigns"]["index"]["get"]>[0]["query"]) => {
        ...
        const test = server.alwaysShowTestTokens ? true : (new URL(request.url).searchParams.get("test") ?? false);
     ...
     * @returns an arr
     */
    const getByOpportunityFromRequest = inject(["server", "request", "api"]).inFunction(
      async ({ server, request, api }, query: Parameters<Api["v4"]["campaigns"]["index"]["get"]>[0]["query"]) => {
        return await fetch(
          async () =>
            await api.v4.campaigns.index.get({ query: getQueryFromRequest.handler({ server, request }, query) }),
        );
      },
    );
