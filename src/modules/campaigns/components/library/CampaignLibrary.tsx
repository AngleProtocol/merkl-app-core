...
export default function CampaignLibrary({
  ...
  const {
    opportunity: opportunityOutlet,
    chain: chainOutlet,
  } = useOutletContext<OutletContextOpportunity>()
  ...
  const nbActiveCampaigns = useMemo(() => {
    ...
    const now = moment().unix()
    const shownCampaigns = opportunity.campaigns.filter(
      (c) => Number(c.endTimestamp) > now,
    )
    return shownCampaigns.length
  }, [opportunity])

  const hasSomeInactiveCampaigns = opportunity.campaigns.some(
    (c) => Number(c.endTimestamp) < moment().unix(),
  )
  const hasOnlyInactiveCampaigns = opportunity.campaigns.every(
    (c) => Number(c.endTimestamp) < moment().unix(),
  )
    ...
    <CampaignTable
      ...
      header={
        <Group className="justify-between items-center w-full">
          <OverrideTheme
            coloring={!hasOnlyInactiveCampaigns ? 'good' : undefined}
          >
            <Group size="xs">
                ...
                helper={
                  <Group>
                    ...
                    <Text look="soft" size={'sm'}>
                      You can earn rewards from multiple campaigns that
                      incentivize the same asset. Check the rules and
