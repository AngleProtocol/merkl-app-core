...
export const MetadataService = defineModule<{ routes: MerklRoutes; backend: MerklBackend; url: string; location: Location }>().create(
  ({ inject }) => {
    /**
    ...
    const fromRoute = <T>(data: {backend: MerklBackend, routes: MerklRoutes, url: string} | undefined, error: unknown, location: Location, callback: (service: typeof MetadataService) => T) => {
      if (error) return [{ title: error }];
      if (!data) return [{ title: error }];
      const { url, backend, routes } = data;

      // return callback(MetadataService({ url, backend: backend as MerklBackend, routes, location }));
    }
