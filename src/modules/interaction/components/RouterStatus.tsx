...
export const interactionRouters = {
  enso: {
    name: "Enso",
    description:
      "Enso provides abstract on-chain actions, shortcuts and routes that allows dApps to find the best routes to interact with other protocols.",
  ...
  zap: {
    name: "Kyberswap Zap",
    description:
      "Zap enables users to effortlessly add liquidity into any concentrated liquidity protocol using any tokens, thanks to the KyberSwap aggregator.",
...
export default function RouterStatus({ name, loading, error }: RouterStatusProps) {
  ...
  return (
    <Dropdown
      ...
      <OverrideTheme coloring={error ? "harm" : undefined}>
        <PrimitiveTag size="sm">
          <Icon src={router?.icon} /> {router.name}
          {loading && <Icon remix="RiLoader2Fill" className="animate-spin" />}
          {error && !loading && <Icon remix="RiCloseFill" />}
        </PrimitiveTag>
      </OverrideTheme>
    </Dropdown>
  );
